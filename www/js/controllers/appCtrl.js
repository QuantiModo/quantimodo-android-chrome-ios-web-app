...
.controller('AppCtrl', function($scope, $timeout, $ionicPopover, $ionicLoading, $state, $ionicHistory, $rootScope,
                                $ionicPopup, $ionicSideMenuDelegate, $ionicPlatform, $injector, qmService, qmLog,
    ...
    $scope.shareStudy = function(correlationObject, sharingUrl, ev){
        if(!correlationObject){
            qmLog.error("No correlationObject provided to shareStudy!");
            return;
        }
        if(!sharingUrl){qmLog.error("No sharing url for this correlation: ", {correlation: correlationObject});}
    ...
    var showUnshareVariableConfirmation = function(variableObject, ev) {
        ...
        function yesCallback() {
            ...
            qmService.postUserVariableDeferred(body).then(function () {}, function (error) {qmLog.error(error);});
        }
    ...
    $rootScope.setLocalStorageFlagTrue = function (flagName) {
        qmLog.debug('Set ' + flagName + ' to true');
    ...
    $scope.downVote = function(correlationObject, $index, ev){
        ...
        if (correlationObject.userVote !== 0) {
            ...
            yesCallback = function() {
                ...
                qmService.postVoteDeferred(correlationObject).then(function () {qmLog.debug('Down voted!');}, function () {qmLog.error('Down vote failed!');});
            };
            noCallback = function() {};
            qmService.showMaterialConfirmationDialog(title, textContent, yesCallback, noCallback, ev);
        } else {
    ...
    $scope.upVote = function(correlationObject, $index, ev){
        ...
        if (correlationObject.userVote !== 1) {
            ...
            yesCallback = function() {
                ...
                qmService.postVoteDeferred(correlationObject).then(function () {qmLog.debug('upVote');}, function () {qmLog.error('upVote failed!');});
            };
            noCallback = function () {};
            qmService.showMaterialConfirmationDialog(title, textContent, yesCallback, noCallback, ev);
        } else {
    ...
    function deleteVote(correlationObject, $index) {
        ...
        qmService.deleteVoteDeferred(correlationObject, function(response){
            ...
            qmLog.error("deleteVote response", response);
        });
    }
    ...
    $scope.onTextClick = function ($event) {
        qmLog.debug("Auto selecting text so the user doesn't have to press backspace...");
        $event.target.select();
    };
    ...
    $scope.trackFavoriteByValueField = function(trackingReminder, $index){
        ...
        qmService.postMeasurementByReminder(trackingReminder, trackingReminder.total)
            .then(function () {
                ...
                qmLog.error('Failed to track favorite! ', trackingReminder);
            });
    };
    ...
    $scope.trackBloodPressure = function(){
        ...
        qmService.postBloodPressureMeasurements($rootScope.bloodPressure)
            .then(function () {
                qmLog.debug("Successfully qmService.postMeasurementByReminder: " + JSON.stringify($rootScope.bloodPressure));
            }, function(error) {
    ...
    $scope.goBack = function (stateParams) {
        if($ionicHistory.viewHistory().backView){
            ...
            qmLog.debug("Going back to " + backView.stateId + "  with stateParams " + JSON.stringify(backView.stateParams));
            $ionicHistory.goBack();
        } else {
            qmService.goToState(config.appSettings.appDesign.defaultState, stateParams);
        }
    ...
    $scope.getUserVariableByName = function (variableName, refresh, hideLoader) {
        if(!variableName){
            qmLog.error('No variable name provided to $scope.getUserVariableByName');
            return;
        }
    ...
    $scope.trackLocationChange = function(event, trackLocation) {
        ...
        qmLog.debug('trackLocation', $rootScope.user.trackLocation);
        ...
        if($rootScope.user && $rootScope.user.trackLocation){
            qmLog.debug('Going to execute qmService.backgroundGeolocationInit if $ionicPlatform.ready');
    ...
    $scope.goToStudyPageWithVariableNames = function(causeVariableName, effectVariableName) {
        qmLog.debug("Clicked go goToStudyPageWithVariableNames for " + causeVariableName + " and " + effectVariableName);
