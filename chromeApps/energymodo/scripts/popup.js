...
var updateMood = function(mood,moodVal,report_time,done){
    ...
    chrome.storage.local.get([config.appSettings.storage_identifier+'allData',
        ...
        if(localStorage[config.appSettings.storage_identifier+'barChartData']){
            var barChartData = JSON.parse(localStorage[config.appSettings.storage_identifier+'barChartData']);
            barChartData[moodVal-1]++;
            var obj = {};
            obj[config.appSettings.storage_identifier+'barChartData'] = JSON.stringify(barChartData);
            chrome.storage.local.set(obj,function(){});
        }

        // update Line chart data
        if(localStorage[config.appSettings.storage_identifier+'lineChartData']){
            var lineChartData = JSON.parse(localStorage[config.appSettings.storage_identifier+'lineChartData']);
            lineChartData.push([report_time, moodVal]);
            var obj = {};
            obj[config.appSettings.storage_identifier+'lineChartData']=JSON.stringify(lineChartData);
            chrome.storage.local.set(obj,function(){});
        }
