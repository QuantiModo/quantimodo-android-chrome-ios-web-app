...
echo "EXECUTING COMMAND: xcodebuild -workspace ${APP_DISPLAY_NAME}.xcworkspace -scheme ${APP_DISPLAY_NAME} -sdk iphoneos -configuration Release  archive -archivePath $PWD/build/${APP_DISPLAY_NAME}.xcarchive CODE_SIGN_IDENTITY=$DEVELOPER_NAME PROVISIONING_PROFILE=$PROFILE_UUID"
...
echo "EXECUTING COMMAND: xcodebuild -exportArchive -archivePath $PWD/build/${APP_DISPLAY_NAME}.xcarchive -exportPath $PWD/build -exportOptionsPlist $PWD/exportOptions.plist"
