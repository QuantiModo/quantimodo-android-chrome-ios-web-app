angular.module('starter').controller('ChartsPageCtrl', function($scope, $q, $state, $timeout, $rootScope, $ionicLoading,  $ionicActionSheet, $stateParams, qmService, qmLog, clipboard) {
    ...
    $scope.$on('$ionicView.enter', function(e) { qmLog.debug("Entering state " + $state.current.name);
        ...
        qmLog.debug("variablePageCtrl: enter");
        ...
        } else {qmLog.error($state.current.name + ' ERROR: $rootScope.variableObject.name not defined!', $rootScope.variableObject);}
        $rootScope.showActionSheetMenu = qmService.variableObjectActionSheet;
    });
    $scope.$on('$ionicView.beforeLeave', function(){
        qmLog.debug('Leaving so setting $scope.stopGettingMeasurements to true');
    ...
    $scope.addNewReminderButtonClick = function() {
        qmLog.debug("addNewReminderButtonClick");
    ...
    var getHistoryForVariable = function(params){
        ...
        if(!params.variableName){
            qmLog.error("ERROR: params.variableName not provided to getHistoryForVariable.  params are: ", params);
            return;
        }
        ...
        qmService.getMeasurementsFromApi(params, function(history){
            ...
            qmLog.error($state.current.name + ' error getting measurements: ' + JSON.stringify(error));
            $scope.state.loadingHistory = false;
        }, function(history) {
            $scope.state.history = $scope.state.history.concat(history);
        });
    ...
    var getDailyHistoryForVariable = function(params){
        if($scope.stopGettingMeasurements){return;}
        if(!params.variableName){
            qmLog.error("ERROR: params.variableName not provided to getHistoryForVariable. params: " + JSON.stringify(params));
            return;
        }
        ...
        qmService.getMeasurementsDailyFromApiDeferred(params).then(function(dailyHistory){
            ...
            qmLog.error($state.current.name + ' error getting dailyHistory measurements: ' + JSON.stringify(error));
            $scope.state.loadingDailyHistory = false;
        }, function(history) {
            $scope.state.loadingDailyHistory = false;
