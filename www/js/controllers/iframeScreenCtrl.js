...
.controller('IframeScreenCtrl', function ($stateParams, $scope, $ionicLoading, $sce, $state, $rootScope, qmService) {
    qmService.logDebug('IframeScreenCtrl works!');
    ...
    if($state.current.name === 'app.studyCreate')
    {
        ...
        iFrameUrl = qmService.getApiUrl() + '/api/v2/studies?hideMenu=true';
    ...
    if($stateParams.path) {
        iFrameUrl = qmService.getApiUrl() + $stateParams.path;
    }
    ...
    {
        iFrameUrl = qmService.getApiUrl() + '/embeddable/?plugin=search-variables';
        $scope.title = 'Your Variables';
    }
    if(window.location.href.indexOf('search-common-relationships') > -1 )
    {
        iFrameUrl = qmService.getApiUrl() + '/embeddable/?plugin=search-relationships&commonOrUser=common';
        $scope.title = 'Common Variable Relationships';
    }
    if(window.location.href.indexOf('search-user-relationships') > -1 )
    {
        iFrameUrl = qmService.getApiUrl() + '/embeddable/?plugin=search-relationships&commonOrUser=user';
        $scope.title = 'Your Variable Relationships';
    }
    if(window.location.href.indexOf('import-data') > -1 )
    {
        iFrameUrl = qmService.getApiUrl() + '/api/v1/connect/mobile';
        $scope.title = 'Your Variable Relationships';
    }
    qmService.logDebug('iframeScreen.init: Going to qmService.getAccessTokenFromAnySource');
    qmService.getAccessTokenFromAnySource().then(function(accessToken) {
        ...
        qmService.logDebug("iframeScreen: No access token. Need to log in.");
