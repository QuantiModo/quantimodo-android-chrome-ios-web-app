...
if [ -z ${BUILD_QUANTIMODO} ];
    then
        ...
        cd /jenkins
        echo "rsync -a --delete --no-perms --omit-dir-times --exclude=/platforms/ --exclude=/build/ ${IONIC_PATH}/* ${PREPARED_APPS_PATH}/${LOWERCASE_APP_NAME}"
        rsync -a --delete --no-perms --omit-dir-times --exclude=/platforms/ --exclude=/build/ ${IONIC_PATH}/* ${PREPARED_APPS_PATH}/${LOWERCASE_APP_NAME}
...
if [ -z ${BUILD_MEDTLC} ];
    then
        ...
        sudo chmod -R 777 ${PREPARED_APPS_PATH}
        cd /jenkins
        echo "rsync -a --no-perms --delete --omit-dir-times --exclude=/platforms/ --exclude=/build/ ${IONIC_PATH}/* ${PREPARED_APPS_PATH}/${LOWERCASE_APP_NAME}"
        rsync -a --delete --no-perms --omit-dir-times --exclude=/platforms/ --exclude=/build/ ${IONIC_PATH}/* ${PREPARED_APPS_PATH}/${LOWERCASE_APP_NAME}
